...
同样，你可以启动一个或多个工作节点，通过下面的命令将它们连接到 master
...
Property Name                                   | Default | Meaning | Since Version
---|:---|:---|:---
`spark.worker.cleanup.enabled`                    | false             | Enable periodic cleanup of worker / application directories. Note that this only affects standalone mode, as YARN works differently. Only the directories of stopped applications are cleaned up. This should be enabled if `spark.shuffle.service.db.enabled` is "true"<br/>启用对worker或应用程序目录的定期清理。注意，这只影响独立模式，因为YARN的工作方式不同。只有已停止应用程序的目录会被清理。这个应该在`spark.shuffle.service.db=true`时启动。| 1.0.0
`spark.worker.cleanup.interval`                   | 1800 (30 minutes) | Controls the interval, in seconds, at which the worker cleans up old application work dirs on the local machine.<br/>worker清理本地机器上的旧应用程序工作目录的时间间隔(以秒为单位)。| 1.0.0
...
spark-submit 脚本是将应用程序提交到集群最直接的方式。对于独立集群有两种部署模式：
...
将下面的命令添加到 `conf/spark-env.sh` 执行以上的操作：
...
为了启用这个恢复模式，可以在 spark-env 中通过配置 `spark.deploy.recoveryMode` 和相关的 `spark.deploy.zookeeper.*` 来设置 `SPARK_DAEMON_JAVA_OPTS`.
...
ZooKeeper 是生产级别的高可用性的最佳方法，但是当 Master 宕掉时，如果你只想能重启 Master 服务器，那么就可用使用 FILESYSTEM 模式。
