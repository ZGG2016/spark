...
```scala
  ...
  // 这就是 Shuffle Write 的缓冲区
  @volatile private[this] var myMemoryThreshold = initialMemoryThreshold

  // Initial threshold for the size of a collection before we start tracking its memory usage
  // For testing only
  // SHUFFLE_SPILL_INITIAL_MEM_THRESHOLD: spark.shuffle.spill.initialMemoryThreshold 默认5mb
 ...
 /**
  ...
  protected def maybeSpill(collection: C, currentMemory: Long): Boolean = {
    ...
    // myMemoryThreshold 就是 Shuffle Write 的缓冲区
    if (elementsRead % 32 == 0 && currentMemory >= myMemoryThreshold) {
      // Claim up to double our current memory from the shuffle memory pool
      // 超过5mb，就会尝试增加到 2*当前使用内存
      val amountToRequest = 2 * currentMemory - myMemoryThreshold
      val granted = acquireMemory(amountToRequest)
      // 更新Shuffle Write 的缓冲区的大小
...
```scala
  ...
  // 当从序列化器读取或写入时，对象批次的大小
  // Objects are written in batches, with each batch using its own serialization stream. This
  // cuts down on the size of reference-tracking maps constructed when deserializing a stream.
  // 对象批量写入，每个批次使用它自己的序列化流。
  // NOTE: Setting this too low can cause excessive copying when serializing, since some serializers grow internal data structures by growing + copying every time the number of objects doubles.
  // 设置得太低会导致在序列化时过度复制，因为一些序列化器通过增长和复制的方式来翻倍内部数据结构
  // SHUFFLE_SPILL_BATCH_SIZE: spark.shuffle.spill.batchSize 默认10000
 ...
 /**
  ...
  private[this] def spillMemoryIteratorToDisk(inMemoryIterator: WritablePartitionedIterator[K, C])
    ...
    // 这个缓冲区是输出流的缓冲区，减少了磁盘搜索和系统调用次数，适当提高可以提升溢写效率
