...
/**
 * 用来跟踪一个 stage 的 map 输出的位置的类。
 * 
 * 这是抽象类，因为 driver 和 executor 有不同的 MapOutputTracker 版本。
 *
 * 原则上， driver 和 executor 端的类不需要共享一个基类。
 * 维护当前的共享基类主要是为了向后兼容，以避免更新现有的测试代码。
 *
...
private[spark] abstract class MapOutputTracker(conf: SparkConf) extends Logging {
   ...
   * endPartition is excluded from the range).(包括startPartition，但不包括endPartition)
   *
   * @return A sequence of 2-item tuples, where the first item in the tuple is a BlockManagerId,
   *         and the second item is a sequence of (shuffle block id, shuffle block size, map index)
   *         tuples describing the shuffle blocks that are stored at that block manager.
   * 一个二元组序列，描述了存储在这个块管理器的 shuffle 块。
