...
object SparkEnv extends Logging {
  ...
  private def create(
    ...
    // 创建给定类名的类的一个实例，可能用我们的 conf 初始化它
    def instantiateClass[T](className: String): T = {
      val cls = Utils.classForName(className)
      // Look for a constructor taking a SparkConf and a boolean isDriver, then one taking just SparkConf, then one taking no arguments
      //寻找一个接收 SparkConf 和布尔类型的 isDriver 的构造函数
      try {
    ...
    // 让用户为 shuffle managers 指定一个简短的名字

    //无论是sort还是tungsten-sort，都取的是SortShuffleManager
    val shortShuffleMgrNames = Map(
      "sort" -> classOf[org.apache.spark.shuffle.sort.SortShuffleManager].getName,
      "tungsten-sort" -> classOf[org.apache.spark.shuffle.sort.SortShuffleManager].getName)
    //获取`spark.shuffle.manager` 设置的 shuffle manager 类型
    val shuffleMgrName = conf.get(config.SHUFFLE_MANAGER)
    val shuffleMgrClass =
    //getOrElse(a,default)：取a对应的值，没有a的话，返回default
      shortShuffleMgrNames.getOrElse(shuffleMgrName.toLowerCase(Locale.ROOT), shuffleMgrName)
    // 根据shuffleMgrClass，创建一个shuffleManager实例
